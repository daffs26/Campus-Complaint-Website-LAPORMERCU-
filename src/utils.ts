...
export const DEFAULT_LAPORAN: Laporan[] = [
  ...
  { id: 3, nim: '2024004', nama: 'Budi', prodi: 'Teknik Mesin', judul: 'Toilet Mampet Lantai 2', kategori: 'Toilet', lokasi: 'Toilet Gedung B Lt.2', deskripsi: 'Toilet mampet dan mengeluarkan bau tidak sedap sudah 2 hari.', tanggal: '2025-01-16', status: 'Belum Diproses' },
  ...
  { id: 5, nim: '2024006', nama: 'Diana', prodi: 'Desain Komunikasi Visual', judul: 'Wifi Perpustakaan Lambat', kategori: 'Perpustakaan', lokasi: 'Perpustakaan Pusat', deskripsi: 'Koneksi wifi di perpustakaan sangat lambat.', tanggal: '2025-01-17', status: 'Belum Diproses' },
  { id: 6, nim: '2024002', nama: 'Daffa Aulia', prodi: 'Sistem Informasi', judul: 'Meja Kantin Rusak', kategori: 'Kantin', lokasi: 'Kantin Utama', deskripsi: 'Banyak meja dan kursi di kantin yang rusak dan kotor.', tanggal: '2025-01-14', status: 'Sedang Diproses' },
];

// Migration map for old status values
const STATUS_MIGRATION: Record<string, Laporan['status']> = {
  'Baru': 'Belum Diproses',
  'Diproses': 'Sedang Diproses',
  'Selesai': 'Selesai',
};

function migrateStatus(list: Laporan[]): Laporan[] {
  let changed = false;
  const migrated = list.map(l => {
    if (STATUS_MIGRATION[l.status as string]) {
      changed = true;
      return { ...l, status: STATUS_MIGRATION[l.status as string] };
    }
    return l;
  });
  if (changed) {
    localStorage.setItem('laporan_mercumb', JSON.stringify(migrated));
  }
  return migrated;
}
...
export function getLaporan(): Laporan[] {
  ...
  return migrateStatus(JSON.parse(data));
}
